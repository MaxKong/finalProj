...
public class Main extends JFrame implements ActionListener{
  ...
  TaskList t;
  Task selected = new Task("1", "071207", "Task1", "121212", 0); //Pull from user input
  ...
  class RemoveTask implements ActionListener {
    public void actionPerformed(ActionEvent event) {
      t.taskList.set( t.taskList.indexOf(selected), new Task("1", "000000", "Done", "121212"));

      listOfTaskButtons.set( listOfTaskButtons.indexOf(selected.name), new JButton( ""));
      refresh();
  ...
  class NewTask implements ActionListener {
    ...
    class SubmitListener implements ActionListener {
      public void actionPerformed(ActionEvent event) {
        ...
        selected = new Task(prioritySelection, (timerHours + timerMinutes + timerSeconds), name, (dueYear + dueMonth + dueDay), index); //THE TIME IS A NUMBER (INTEGER)
        ...
        listOfTaskButtons.get(index).addActionListener(new ClickResponse(index)); //Adds Index Value of Task
        index++;
  ...
  class ClickResponse implements ActionListener {
    ...
    public void actionPerformed(ActionEvent event) {
      selected = t.taskList.get(t.taskList.get(i).index);
      timer = new Timer( ((selected.time)/10000), (((selected.time)%10000)/100), ((selected.time)%100));
